...
- Use shadcn/ui components from `packages/desktop/src/renderer/src/components/ui` for reusable UI primitives.
- Keep feature UI in `packages/desktop/src/renderer/src` and platform/service logic in `packages/desktop/src/main/services`.
...
- When a feature needs a new visual pattern, decide whether it belongs in `packages/desktop/src/renderer/src/components/ui` or a feature-local component before adding styles inline.
