...
const diffDashMcpTools = claudeMcpToolNames([
  "getReviewContext",
  "getChangedFiles",
  "searchReviewDiff",
  "getDiffHunk",
...
/** Legacy review-provider layer backed by the extracted Claude SDK provider. */
export const claudeReviewAgentLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const registration = makeClaudeProvider({ cli, cliStream })
    const review = registration.reviewThread
    if (review === undefined) throw new Error("Claude review capability is not registered")
    ...
    const runThreadTurn = Effect.fn("ClaudeReviewAgent.runThreadTurn")(function* (
      ...
      if (input.cwd === null) {
        return yield* ReviewAgentPermissionError.make({
          provider: providerId,
          reason: "Claude review execution requires an isolated working directory",
        })
      }
  ...
  mcp: {
    ...
    call: (_call: ScopedMcpCall) =>
      AgentProviderOperationError.make({
        ...
        reason: "Claude accesses MCP through its scoped transport",
      }),
  },
...
const mapProviderError = (
  ...
  if (
    /non-mutating policy|MCP access includes tools|permission|sensitive-file/iu.test(cause.reason)
  ) {
    return ReviewAgentPermissionError.make({ provider: providerId, reason: cause.reason })
  }
  if (/stream-json|result event|omitted/iu.test(cause.reason)) {
