...
import { Context, Effect, Layer, Option, Ref, Schema, Semaphore, type Scope } from "effect"
...
export const coreReviewDataWorkerLayer = (options: CoreReviewDataWorkerOptions) =>
  Layer.effect(
    CoreReviewDataWorker,
    Effect.gen(function* () {
      const active = yield* Ref.make<Option.Option<ReviewDataWorkerClient>>(Option.none())
      ...
      const process = Effect.fn("CoreReviewDataWorker.process")(function* (
        ...
        yield* switchLock.withPermits(1)(
          Ref.getAndSet(active, Option.some(client)).pipe(
            Effect.flatMap((previous) =>
              Option.match(previous, {
                onNone: () => Effect.void,
                onSome: (worker) => Effect.promise(() => worker.dispose()),
              }),
            ),
          ),
        )
        yield* Effect.addFinalizer(() =>
          switchLock.withPermits(1)(
            Ref.modify(active, (current) => [
              Option.exists(current, (worker) => worker === client),
              Option.filter(current, (worker) => worker !== client),
            ]).pipe(
        ...
        yield* consumeReviewDiffSource(source, acquisition, client)
      })
