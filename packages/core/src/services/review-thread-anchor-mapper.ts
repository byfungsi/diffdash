...
type AnchorMapping = typeof CurrentReviewAnchor.Type
...
const toCurrentMapping = (
  ...
  currentBaseRevision,
  currentHeadRevision,
  currentAnchor: mapAnchor(thread, diff),
})
...
const mapLineAnchor = (anchor: LineReviewAnchor, file: ParsedDiffFile): AnchorMapping => {
  ...
  if (hunkMatch.kind === "missing") return CurrentReviewAnchor.cases.Unresolved.make({})
  ...
  if (lineNumber === null) return CurrentReviewAnchor.cases.Unresolved.make({})
  if (lineNumber === undefined) return CurrentReviewAnchor.cases.Unresolved.make({})
