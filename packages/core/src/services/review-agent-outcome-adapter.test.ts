...
import { ReviewAgentArtifact, ReviewThreadAgentResponse } from "@diffdash/domain/review-agent"
...
describe("adaptReviewAgentOutcome", () => {
  it.effect("converts agent artifacts into bounded persisted review-agent data", () => {
    ...
    const agentOutcome: ReviewThreadAgentOutcome = {
    ...
    return Effect.gen(function* () {
      ...
      const result = yield* adaptReviewAgentOutcome(
  ...
  it.effect("preserves a response with no optional domain fields", () =>
    Effect.gen(function* () {
      const normalizer = yield* AgentArtifactNormalizer
      const result = yield* adaptReviewAgentOutcome(
        AgentProviderId.make("codex"),
