  ...
  it.effect("FUN-66 AC: maps all anchor outcomes deterministically and idempotently", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        expect(byName.get("retained")?.activeAnchor).toBeInstanceOf(LineReviewAnchor)
        expect(byName.get("retained")).toMatchObject({ currentAnchor: { _tag: "Active" } })
        expect(byName.get("shifted-line")).toMatchObject({
          currentAnchor: { _tag: "Active", anchor: { _tag: "line", lineNumber: 25 } },
        })
        expect(byName.get("changed")).toMatchObject({
          currentAnchor: { _tag: "Unresolved" },
        })
        ...
        expect(byName.get("renamed")).toMatchObject({
          currentAnchor: {
            _tag: "Active",
            anchor: { _tag: "line", filePath: "src/new-name.ts", oldPath: "src/old-name.ts" },
          },
        })
        expect(byName.get("changed")).toMatchObject({
          baseRevision: originalBaseRevision,
