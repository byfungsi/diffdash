...
const makeMessage = (
  ...
  if (author === "user") {
    return UserReviewThreadMessage.make({ ...identity, bodyMarkdown: MarkdownBody.make(body) })
  }
  const agentRunId = AgentRunId.make(`run-${sequence}`)
  if (status === "pending") {
    return PendingAgentReviewThreadMessage.make({ ...identity, agentRunId })
  }
  if (status === "failed") {
    return FailedAgentReviewThreadMessage.make({
      ...identity,
      agentRunId,
      failure: InternalReviewThreadMessageFailure.make({}),
    })
  }
  return CompletedAgentReviewThreadMessage.make({
    ...identity,
