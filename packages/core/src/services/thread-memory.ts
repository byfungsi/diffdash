...
type CompletedThreadMessage = UserReviewThreadMessage | CompletedAgentReviewThreadMessage
...
interface ThreadMemoryWindow {
  readonly memory: ThreadMemory | null
  readonly messages: readonly CompletedThreadMessage[]
}
...
export const createFallbackThreadMemoryUpdate = (
  ...
  const completed = orderedCompletedMessages(input.threadId, input.messages)
  const latestSuccessfulReply = Option.getOrUndefined(
    Array.findLast(completed, (message) =>
...
const orderedCompletedMessages = (
  ...
  Array.sortBy(
    ...
    messages.filter(
      (message): message is CompletedThreadMessage =>
...
const renderMessage = (message: CompletedThreadMessage) => {
  const author = Match.valueTags(message, {
    User: () => "user",
    Completed: () => "assistant",
  })
