...
export class ProgressiveReviewContentSession implements ProgressiveReviewContentReader {
  ...
  readonly #files = new ReviewPageCache()
  ...
  /** Returns the currently retained parsed file. */
  readonly getFile = (fileId: ReviewFileId): ParsedDiffFile | null => this.#files.get(fileId)
  ...
  readonly #startFileLoad = (fileId: ReviewFileId): Promise<ProgressiveReviewFileLoadStatus> => {
    ...
    const promise = this.#readFile(generation, identity, fileId, controller.signal)
      .then((file) => {
        ...
        if (!this.#files.put(file)) throw new Error("Diff file exceeds the renderer cache budget")
        this.#publish({ ...this.getProjection(), files: this.#files.files() })
  ...
  readonly #readFile = async (
    ...
    while (this.#isIdentityCurrent(generation, identity) && !signal.aborted) {
      // oxlint-disable-next-line eslint/no-await-in-loop -- Legal persisted ranges must be consumed in line order.
      ...
      if (!this.#isIdentityCurrent(generation, identity) || !sameIdentity(range.identity, identity))
        return null
      for (const block of range.blocks) chunks.push(block.bytes)
    ...
    const patch = chunks.map((chunk) => decoder.decode(chunk)).join("")
