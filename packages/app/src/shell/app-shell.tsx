...
import { buildReviewFileTreeInput } from "@/review/file-tree-adapter"
...
import { reviewRequestsAtom, repoPrCountsAtom } from "@/home/atoms"
...
import {
  hostedReviewSnapshotAtom,
  localReviewSnapshotAtom,
  pullRequestAtomKey,
  pullRequestsAtom,
...
export function AppShell() {
  ...
  const selectedHostedReviewSnapshotAtom = hostedReviewSnapshotAtom(selectedPullRequestReviewKey)
  ...
  const selectedHostedReviewResult = useAtomValue(selectedHostedReviewSnapshotAtom)
  ...
  const refreshSelectedHostedReview = useAtomRefresh(selectedHostedReviewSnapshotAtom)
  ...
  const selectedHostedReviewSnapshot = resultValue(selectedHostedReviewResult, null)
  const selectedPullRequest = selectedHostedReviewSnapshot?.detail ?? null
  const selectedPullRequestDiff = selectedHostedReviewSnapshot?.parsedDiff ?? null
  ...
  const reviewStatus =
    selectedReview === null
      ...
      : selectedReview.kind === "localDiff"
        ...
        : Result.isFailure(selectedHostedReviewResult)
          ? resultErrorMessage(selectedHostedReviewResult, "Could not open pull request")
          : selectedPullRequest === null
            ? `Opening ${reviewProvider?.terminology.reviewAbbreviation ?? "review"} #${selectedReview.number}...`
            : Result.isWaiting(selectedHostedReviewResult)
  ...
  useEffect(() => {
    ...
    if (Result.isFailure(selectedHostedReviewResult)) {
      setActionStatus(resultErrorMessage(selectedHostedReviewResult, "Reload diff failed"))
      setIsReloadingReview(false)
      return
    }
