...
export const ReviewCommentsCodeSourceContribution = ({ source }: CodeSourceContributionProps) => {
  ...
  const matchingDraft = Option.filter(codeDraft, (draft) => codeDraftMatchesSource(draft, source))
  const annotationLine = Option.match(matchingDraft, {
    onNone: () => null,
    onSome: (draft) => draft.lineNumber,
  })
  useEffect(() => {
  ...
  const output = useMemo(
    ...
    [annotationLine, toggleCodeDraft],
  )
...
const CodeCommentAnnotation = () => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (body.length === 0 || revision === null || submitting) return
    setSubmitting(true)
    setError(null)
    try {
      ...
      {CommentDestination.match(commentSubmission.destination, {
        ...
        OpenCode: () =>
          Option.match(draft.gitRevision, {
            ...
            onSome: () => (
              <form className="bg-card space-y-2 rounded-lg border p-3 shadow-xs" onSubmit={submit}>
                <label className="block text-xs font-semibold">
                  ...
                  <Textarea
                    ...
                    value={draft.body}
                    onChange={(event) =>
                      reviewComments.updateCodeDraftBody(event.currentTarget.value)
                    }
                    onKeyDown={(event) => {
                ...
                <div className="flex justify-end gap-1.5">
                  ...
                  <Button
                    type="submit"
                    size="xs"
                    disabled={draft.body.trim().length === 0 || submitting}
                  >
