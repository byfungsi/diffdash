...
The wizard has completed integration of PostHog analytics into the DiffDash landing page. The project already had a custom lightweight analytics module (`web/landing/src/analytics.ts`) that sends events directly to PostHog's capture API using `sendBeacon` / `fetch` with `keepalive` — a deliberately minimal approach suitable for a one-page landing site deployed on Cloudflare Workers. The wizard preserved this implementation, set up the required environment variables, and added one new event to track navigation intent.
...
- [ ] Run a full production build (`pnpm build` in `web/landing`) and fix any lint or type errors introduced by the generated code.
- [ ] Run the test suite — call sites that were rewritten or instrumented may need updated mocks or fixtures.
- [ ] Add `VITE_POSTHOG_KEY` and `VITE_POSTHOG_HOST` to `web/landing/.env.example` (they are already present there) and confirm any CI/CD or Cloudflare Pages environment configuration includes both variables so events are sent from deployed previews and production.
