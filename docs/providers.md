...
are leaf packages imported only by the Core composition root. See [Architecture](architecture.md)
...
prove that a provider can be introduced with one package and one explicit Core registration.
